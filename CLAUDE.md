...
- **The README carries only what is expensive not to know.** Signatures, fields, defaults and the sentinel list belong in godoc, and reasoning belongs in this file; a section that restates an API is noise, and it stops being read at all once it reads like a reference. Adding a `Config` field or a sentinel is therefore a godoc edit, not a README edit — touch the README only when the *trap* changes. The one comparison table this repo owns is "genai-kit vs go-gemini-client": it lives here and go-gemini-client links to it, so a table in both places cannot go stale on one side (`public-docs/docs/library-readme-convention.md` §2.6). Sample code names concrete models; refresh them when the current generation moves on.
- **`imagegen` deliberately has no rate limiting of its own**, which is why the README tells callers to decorate `imagegen.Generator` with `callguard` at the workflow layer. Vertex quota is per project, so a limiter inside each library sums past the quota; the sharing has to happen above both. Don't add `WithRateInterval` here to make the package feel complete.
